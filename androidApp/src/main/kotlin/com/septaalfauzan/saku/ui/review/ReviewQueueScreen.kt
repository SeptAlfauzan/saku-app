                ...
                LazyColumn(
                    ...
                    items(state.pending, key = { it.id }) { tx ->
                        Column {
                            TransactionRow(tx)
                            Row(
                                Modifier.padding(start = SakuDp.spaceXs, top = SakuDp.spaceXs),
                                horizontalArrangement = Arrangement.spacedBy(space = SakuDp.spaceSm)
                            ) {
                                PillButton(
                                    stringResource(R.string.review_confirm),
                                    icon = Icons.Default.CheckCircle,
                                    onClick = { viewModel.confirm(tx.id) },
                                ...
                                PillButton(
                                    ...
                                    onClick = { viewModel.ignore(tx.id) },
